	...
	if(isset($_POST['envoyer'])){//si le bouton envoyer (name="envoyer") est cliqué, on traite le fomulaire
		if(empty($_POST['commentaire'])){
			...
			if ($_SESSION['connecté'] != 1){
				$msg_erreur.="Vous n'êtes pas connecté<br/>";
			}

			if ($_SESSION['adejacommente'] == 0){
				$msg_erreur.="Vous avez déjà commenté!<br/>";
			}
			...
			if(stristr($_POST['commentaire'],"pute")){//désolé c'est pour l'exemple :) ici j'utilise stristr qui est moins gourmand que preg_match, stristr recherche le mot en version Insensible (str"i"str), il trouvera connard comme Connard
				$msg_erreur.="C'est toi la pute<br/>";
			}
		...
		if($msg_erreur!=''){
			...
			$Pseudo = htmlent($_SESSION['pseudo']);//voir la fonction htmlent() dans le fichier fonctions/fonctions.php pour plus de détails
			$Mail = htmlent($_SESSION['email']);
			...
			if(mysqli_query($mysqli,"INSERT INTO commentaires SET 
				...
				date = ".time())){//time() donne le timestamp actuel, on pourra le manipuler avec la fonction date(), exemple: date("H:i d-m-Y",quand)
				echo "<p>Commentaire posté avec succès!</p>";
				$_SESSION['adejacommente'] = 0;
				...
				$_SESSION['AfficherForm'] = 0;//on cache le formulaire
			} else {
...
<!-- 			Pseudo
			...
			<?php
			if ($_SESSION['connecté'] == 1){
				echo "<input type='submit' name='envoyer' value='Poster!'>";
			 }
			else{
...
function afficherCommentaires($IdArticle=0){
	...
	if(mysqli_num_rows($req)==0) {
		...
		<div style="margin-bottom:30px;border-bottom:1px solid black">
			<p style="text-decoration:underline"><b><?php echo $infos['pseudo']; ?> le <?php echo date("d-m-Y",$infos['date'])." à ".date("H:i",$infos['date']); ?></b></p>
